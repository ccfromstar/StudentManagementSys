...
$(function(){
			...
			checkuserAction(info,'学员出勤','学员出勤');
			...
			$('#test').datagrid({
				...
				url:'DataPage/getDatasBysite.jsp?&p=arrivedinfo&q=2@3@4&r=ArrSite&s='+site,
				sortName: 'p0',
				sortOrder: 'desc',
				idField:'p0',
				frozenColumns:[[
			...
			$('#test').datagrid({
				...
				nowrap: false,
				striped: true,
				url:'DataPage/getDatas.jsp?&p=arrivedinfo&p=1@2@3@4@5',
				sortName: 'p0',
				sortOrder: 'desc',
				idField:'p0',
				frozenColumns:[[
	                {field:'ck',checkbox:true},
	                {field:'p0',title:'考勤编号',width:pw*0.1},
	                {field:'p1',title:'考勤学员卡号',width:pw*0.2},
					{field:'p2',title:'考勤课程',width:pw*0.2},
					{field:'p3',title:'考勤日期',width:pw*0.3-80},
					{field:'p4',title:'考勤地点',width:pw*0.2}
				]],
				pagination:true,
				rownumbers:true,
				singleSelect:true,
			...
			$("#searchuser").click(function(){
				...
				if($('#cour').combobox('getValue') == ""){
					$.messager.alert('提示','考勤课程不能为空!','warning');return false;
				}
				//通过Ajax来读取要编辑的记录
...
<div id="form1" region="north" title="学员出勤" split="true"
...
<a class="easyui-linkbutton" id="adduser" icon="icon-add"
	href="javascript:void(0)">考 勤</a>
	<input type="hidden" name="tablename" value="arrivedinfo" />
	<input type="hidden" name="label" value="0" />
	<input type="hidden" name="returnpage" value = "StuArrived.jsp" />
	<input type="hidden" name="fields" value="ArrNo,ArrCourse,ArrData,ArrSite" />
...
<form name="delrecord" method="post" action="FunctionPage/delRecord.jsp"
	...
	<input type="hidden" name="deltablename" value="arrivedinfo" />
	<input type="hidden" name="delreturnpage" value="StuArrived.jsp" />
	</form>
</div>
