...
$(function(){
			...
			if(s != "管理员"){
				...
				nowrap: false,
				striped: true,
				url:'DataPage/getDatas.jsp?&p=topicinfo&p=1@2@4@5',
				sortName: 'p0',
				sortOrder: 'desc',
				idField:'p0',
				frozenColumns:[[
	                 {field:'p1',title:'主题',width:pw*0.4,formatter:function(value,rec){
							return '<span style="color:blue"><a href="#">'+value+'</a></span>';
						}
					},
					{field:'p2',title:'发布人',width:pw*0.3},
					{field:'p3',title:'发布日期',width:pw*0.4-80}
				]],
				pagination:true,
				rownumbers:true,
				singleSelect:true,
				...
				onClickRow:function(rowIndex){ 
                    var rows = $('#test').datagrid('getSelections');
						...
						window.location = "Topic.jsp?p="+rows[0].p0; 
                } ,
				toolbar:[{
					text:'新建主题',
					iconCls:'icon-add',
			...
			$('#test').datagrid({
				...
				frozenColumns:[[
					...
					{field:'p2',title:'发布人',width:pw*0.2},
					{field:'p3',title:'发布日期',width:pw*0.3-80}
				]],
				pagination:true,
				rownumbers:true,
				singleSelect:true,
...
<div id="w" class="easyui-window" minimizable="false" maximizable="false" collapsible="false" closed="true" title="添加主题"
	icon="icon-save"
	style="width: 720px; height: 240px; padding: 5px; background: #fafafa;">
...
<table border="1" width="98%">
	...
	<tr>
		...
		<div style="width: 100%; text-align: left;"><textarea
			...
			style="height: 50px; width: 500px"></textarea></div>
		</td>
	</tr>
	<tr>
		<td>发布人:</td>
...
<a class="easyui-linkbutton" id="adduser" icon="icon-add"
	href="javascript:void(0)">添加主题</a> <input type="hidden"
	name="topicperson" value=<%=session.getAttribute("userName") %> /> <input
	type="hidden" name="topicdate" /> <input type="hidden"
	name="tablename" value="topicinfo" /> <input type="hidden"
	name="label" value="0" /> <input type="hidden" name="returnpage"
	value="Discuss.jsp" /> <input type="hidden" name="fields"
	value="TopicTitle,TopicMain,TopicPerson,TopicData" /> <input
	type="hidden" name="postparams"
	value="topictitle;topicmain;topicperson;topicdate" /> <input
	type="hidden" name="types" value="0;0;0;0" /></form>
<form name="delrecord" method="post" action="FunctionPage/delRecord.jsp"
	style="display: none">
	<input type="hidden" name="idstr" />
	<input type="hidden" name="deltablename" value="topicinfo" />
	<input type="hidden" name="delreturnpage" value="Discuss.jsp" />
	</form>
</div>
</div>
</div>
