...
String sql = "SELECT * FROM topicinfo where id=" + idstr;
...
if (stmt != null) {
	...
	//查找回复的信息
	sql = "SELECT * FROM responseinfo where ParentID=" + idstr;
	rs = stmt.executeQuery(sql);//执行sql语句
	if (rs != null) {
		while (rs.next()) {
			al.add(rs.getString(2));
			bl.add(rs.getString(3));
			cl.add(rs.getString(4));
		}
	}
	//用JSP拼接html
	for(int i=0;i<al.size();i++){
		html = html + "<table width='97%' border='1'>";
		html = html +	"<tr><td><font style='color:blue'><b>回复:</b></font>"+al.get(i)+"<br/><font style='color:blue'><b>回复人:</b></font>"+bl.get(i)+"<br/><font style='color:blue'><b>回复日期:</b></font>"+cl.get(i)+"<br/></td></tr>";
		html = html + "</table>";
	}
}
...
$(function(){
			...
			txtHTML = txtHTML +	"<tr align='left'><td><font style='color:blue'><b>主题:</b></font><%=p0%><br/><font style='color:blue'><b>内容:</b></font><%=p1%><br/><font style='color:blue'><b>发布人:</b></font><%=p2%><br/><font style='color:blue'><b>发布日期:</b></font><%=p3%></td></tr>";
...
<div id="view" region="north" title="主题内容" split="true"
	style="height: 280px; overflow: auto;text-align:center">
	<div id="topiclist"></div>
</div>
<div id="form1" region="center" title="回复"  split="true" style="overflow: hidden;">
<table id="test"></table>
<form name="userform" method="post" action="FunctionPage/insertResponse.jsp"
	id="userform" style="padding: 0px; margin: 5px;">
