...
$(function(){
			...
			checkuserAction(info,'学员卡状态','学员卡状态');
			loadsiteInfo();
			loadcourseInfo();
			loadcardInfo();
			...
			$("#addmoney").click(function(){
				if($("select[name='sex']").val()==$("input[name='stucrdstatus']").val()){
					$.messager.alert('提示','不能转成当前状态,当前卡已经'+$("select[name='sex']").val()+'!','warning');return false;
				}
				if($("input[name='stuid']").val()==""){
					$.messager.alert('提示','请选择要挂失的学员!','warning');return false;
				}
...
<div id="form1" region="north" title="学员卡挂失" split="true"
...
<table id="tb1" border="1" width="100%">
	...
	<tr>
		...
		<td width="12.5%"><div style="text-align:left"><select name="sex" id="sex">
...
<br />
<br />
&nbsp;&nbsp;<a class="easyui-linkbutton" id="addmoney" icon="icon-save"
	href="javascript:void(0)">调整状态</a> <br />
<input type="hidden" name="stusite" disabled
	style="background-color: white; border: none; text-align: left" /> <input
	type="hidden" name="stubrithday" disabled
	style="background-color: white; border: none; text-align: left" /> <input
	type="hidden" name="stuid" readonly
	style="background-color: white; border: none; text-align: left" /> <input
	type="hidden" name="id1" /> <input type="hidden" name="tablename1"
	value="stuinfo" /> <input type="hidden" name="returnpage1"
	value="StuCrdLoss.jsp" /> <input type="hidden" name="fields1"
	value="StuCrdStatus" /> <input type="hidden" name="postparams1"
	value="sex" /> <input type="hidden" name="types1" value="0" /></form>
